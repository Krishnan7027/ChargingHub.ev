...
const speedColors: Record<string, string> = {
  ...
  'Standard': 'glass text-theme-secondary',
    ...
    <Link href={`/stations/${rec.stationId}`} className="card hover:shadow-md transition-shadow block">
      <div className="flex items-start gap-3">
        ...
        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
          rank === 1 ? 'bg-yellow-100 text-yellow-700' : rank === 2 ? 'glass text-theme-secondary' : rank === 3 ? 'bg-orange-100 text-orange-700' : 'bg-theme-secondary text-theme-muted'
        }`}>
          {rank}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-semibold text-theme-primary truncate">{rec.name}</h3>
            {rank <= 3 && (
          ...
          <p className="text-sm text-theme-secondary">{rec.address}, {rec.city}</p>

          <div className="flex flex-wrap gap-2 mt-2">
            <span className="text-xs glass text-theme-secondary px-2 py-0.5 rounded-full">
              {rec.distanceKm} km
            </span>
            <span className={`text-xs px-2 py-0.5 rounded-full ${speedColors[rec.speedLabel] || 'glass text-theme-secondary'}`}>
          ...
          <div className="flex gap-1 mt-2">
            {Object.entries(rec.scoreBreakdown).map(([key, val]) => (
              <div key={key} className="flex-1">
                ...
                <p className="text-[9px] text-theme-muted mt-0.5 text-center capitalize">{key}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-right flex-shrink-0">
          ...
          <p className="text-[10px] text-theme-muted">score</p>
          {rec.rating > 0 && (
            <div className="flex items-center gap-0.5 mt-1 justify-end">
              ...
              <span className="text-xs text-theme-secondary">{rec.rating.toFixed(1)}</span>
            </div>
          )}
        </div>
      </div>
