...
const statusColors: Record<string, { bg: string; text: string; ring: string }> = {
  normal: { bg: 'bg-green-50', text: 'text-green-700', ring: 'stroke-green-500' },
  warning: { bg: 'bg-yellow-50', text: 'text-yellow-700', ring: 'stroke-yellow-500' },
  critical: { bg: 'bg-orange-50', text: 'text-orange-700', ring: 'stroke-orange-500' },
  emergency: { bg: 'bg-red-50', text: 'text-red-700', ring: 'stroke-red-500' },
};
  ...
  return (
    <div className="card">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-semibold text-gray-900">Grid Load</h3>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 space-y-1.5">
          ...
          <div className="flex justify-between text-xs">
            <span className="text-gray-500">Grid Capacity</span>
      ...
      {data.slots.length > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <p className="text-xs text-gray-500 mb-2">Slot Power Draw</p>
          <div className="space-y-1">
            {data.slots.filter((s) => s.isCharging).map((slot) => (
              <div key={slot.slotId} className="flex items-center gap-2">
                <span className="text-[10px] text-gray-400 w-12">Slot {slot.slotNumber}</span>
