  ...
  return (
    <div className="card">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-semibold text-theme-primary">Carbon Impact</h3>
      </div>

      {isLoading ? (
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-8 glass rounded animate-pulse" />
          ))}
        </div>
      ) : !data?.totals ? (
        <p className="text-sm text-theme-muted text-center py-4">No carbon data yet</p>
      ) : (
        <>
          {/* Hero stat */}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-theme-secondary rounded-lg p-2.5 text-center">
              <p className="text-lg font-bold text-blue-600">
                {data.totals.energyKwh.toFixed(1)}
              </p>
              <p className="text-[10px] text-theme-secondary">kWh Delivered</p>
            </div>
            <div className="bg-theme-secondary rounded-lg p-2.5 text-center">
              <p className="text-lg font-bold text-emerald-600">
                {data.totals.treesEquivalent.toFixed(1)}
              </p>
              <p className="text-[10px] text-theme-secondary">Trees Equivalent</p>
            </div>
            <div className="bg-theme-secondary rounded-lg p-2.5 text-center">
              <p className="text-lg font-bold text-purple-600">
                {data.totals.milesOffset.toFixed(0)}
              </p>
              <p className="text-[10px] text-theme-secondary">Gas Miles Offset</p>
            </div>
            <div className="bg-theme-secondary rounded-lg p-2.5 text-center">
              <p className="text-lg font-bold text-orange-600">
                {data.totals.sessions}
              </p>
              <p className="text-[10px] text-theme-secondary">Sessions Tracked</p>
            </div>
          </div>
          ...
          {data.dailyTrend && data.dailyTrend.length > 0 && (
            <div className="mt-4">
              <p className="text-xs text-theme-secondary mb-2">Daily Carbon Savings</p>
