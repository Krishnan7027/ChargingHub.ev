...
export default function AuthModal({ open, onClose, onAuthenticated, initialMode = 'login' }: AuthModalProps) {
  ...
  // Set mode when modal opens, reset when closes
  useEffect(() => {
    if (open) {
      setMode(initialMode);
    } else {
  ...
  function switchMode(newMode: 'signup' | 'login' | 'otp') {
    resetForm();
    setMode(newMode);
  }
    ...
    <Modal open={open} onClose={onClose} maxWidth="max-w-sm">
      <div className="text-center mb-5">
        ...
        <h2 className="text-xl font-bold text-theme-primary">
          {mode === 'signup'
            ? 'Get Started with EV Charge Hub'
            : mode === 'otp'
              ? 'Login with OTP'
              : 'Welcome back'}
        </h2>
        ...
        otpSent ? (
          <form onSubmit={handleVerifyOtp} className="space-y-3">
            <div>
              ...
              <p className="text-sm text-primary-500 font-medium mb-3">{otpIdentifier}</p>
              <input
                ref={otpInputRef}
                type="text"
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {mode === 'signup' && (
              <>
                ...
                <div>
                  <label className="block text-sm font-medium text-theme-secondary mb-1">Phone <span className="text-theme-muted">(optional)</span></label>
                  <input
                    type="tel"
                    className="input"
            ...
            {mode === 'signup' && (
              <div>
                <label className="block text-sm font-medium text-theme-secondary mb-1">I am a...</label>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button
                      ...
                      disabled={loading}
                      className={`p-2.5 rounded-xl border-2 text-center transition-all disabled:opacity-50 ${
                        role === opt.value
